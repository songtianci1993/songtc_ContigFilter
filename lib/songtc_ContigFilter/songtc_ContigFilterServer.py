...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'songtc_ContigFilter'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from songtc_ContigFilter.songtc_ContigFilterImpl import songtc_ContigFilter
impl_songtc_ContigFilter = songtc_ContigFilter(config)
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'songtc_ContigFilter'
        ...
        self.rpc_service.add(impl_songtc_ContigFilter.filter_contigs,
                             name='songtc_ContigFilter.filter_contigs',
                             types=[dict])
        self.method_authentication['songtc_ContigFilter.filter_contigs'] = 'required'
        self.rpc_service.add(impl_songtc_ContigFilter.status,
                             name='songtc_ContigFilter.status',
                             types=[dict])
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "songtc_ContigFilter but no authentication header was passed"
                            raise err
